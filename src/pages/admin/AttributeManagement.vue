...
import { getAllColorsApi, createColorApi, updateColorApi, deleteColorApi, getAllSizesApi, deleteSizeApi, createSizeApi, updateSizeApi } from '../../api/attributeApi';
// Removed ConfirmationModal import - now using native browser confirm dialogs
import ColorEditModal from './components/ColorEditModal.vue';
import SizeEditModal from './components/SizeEditModal.vue';
...
const handleSaveColor = async (colorFormDataFromModal) => {
  ...
  try {
    if (isEditModeColor.value) {
      await updateColorApi(payload.id, payload); // Pass payload without 'value'
      alert('颜色更新成功');
    } else {
      await createColorApi(payload); // Pass payload without 'value'
      alert('颜色添加成功');
    }
    fetchColors();
    closeColorModal();
...
.attribute-table th {
  background-color: transparent; /* Match ProductManagement table style */
  color: var(--color-text-secondary);
...
.attribute-table tbody tr:hover td {
  background-color: rgba(51, 65, 85, 0.5); /* Match ProductManagement table style */
